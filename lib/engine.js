/*
 ...
 (see the comment at https://gitlab.com/alpinebits/rtapp-201507/issues/1#note_78771494 )
...
function run(job) {
    ...
    if (job.protocol_version === undefined) {
        job.protocol_version = '2017-10';
    }
    job.protocol_version = String(job.protocol_version);
    if (job.protocol_version !== '2017-10' && job.protocol_version !== '2018-10' && job.protocol_version !== '2020-10') {
        throw 'job parameter: if given, protocol_version must be 2017-10, 2018-10 or 2020-10';
    }
    ...
    var plans = find_plans(rpmsg, job.protocol_version);
    ...
    for (i = 0; i < plans.length; i++) {
        ...
        var currency = plans[i].$.CurrencyCode;
        ...
        for (c = 0; c < rate_codes.length; c++) {
            ...
            for (k = 0; k < rates.length; k++) {
                ...
                obj = utilrate.get_rate(rates[k], static_data, job.protocol_version);
                ...
                Object.keys(obj.base_amt).forEach(function (el) {
                    info1('            | BaseByGuestAmt: ' + el + ' pax -> ' + obj.base_amt[el].amtat + ' ' + currency);
                });
                obj.add_amt.forEach(function (el) {
                    if (el.agecode === '10') {
                        info1('            | AdditionalGuestAmounts: adult -> ' + el.amount + ' ' + currency);
                    } else {
                        info1('            | AdditionalGuestAmounts: agecode = ' + el.agecode + ', ' + el.minage + ' <= age < ' + el.maxage + ' -> ' + el.amount + ' ' + currency);
                    }
                });

            }
            utilrate.check_rates_overlap(rates, static_data, job.protocol_version);
        ...
        for (c = 0; c < supp_codes.length; c++) {
            ...
            for (k = 0; k < merged_supp.dyn.length; k++) {
                info1('            | '
                    ...
                    + ' -> ' + merged_supp.dyn[k].amount + ' ' + currency + ' ' + 
    ...
    for (i = 0; i < plans.length; i++) {
        ...
        for (c = 0; c < rate_codes.length; c++) {
            ...
            if (io.mch === undefined) {
                ...
                // expression has changed in 2018-10
                if (job.protocol_version === '2017-10') {
                    full_payers_needed = Math.max(io.min, Math.min(io.max - io.mch, io.std));
                } else {
                    full_payers_needed = Math.min(io.max - io.mch, io.std);
                }
            }
            ...
            var rmatch = match_rates(job.arrival, job.departure, eff_adult, eff_child, io, plans[i], rate_codes[c], discount, num_applied, job.protocol_version, currency);
            ...
            info2('        +-- matching rates for the stay (total contribution ' + round3(rmatch.cost_total) + ' ' + currency + '):');
            ...
            if (smatch.cost_details.length === 0) {
                ...
                info2('        +-- matching, mandatory supplements for the stay (total contribution ' + round3(smatch.cost_total) + ' ' + currency + '):');
                for (k = 0; k < smatch.cost_details.length; k++) {
                    info2('            +-- ' + smatch.cost_details[k]);
                }
            }

            info2('        +-- total cost: ' + round2(rmatch.cost_total + smatch.cost_total) + ' ' + currency);
...
function find_plans(rpmsg, protocol_version) {
    ...
    for (i = 0; i < rp.length; i++) {
        ...
        if (protocol_version === '2017-10' || protocol_version === '2018-10') {
            // in versions before 2020-10, currency code must be EUR...
            if (rp[i].$.CurrencyCode !== 'EUR') {
                throw 'invalid RatePlan: CurrencyCode must be "EUR"';
            }
        } else {
            // ...in 2020-10 this was relaxed to any three character string
            if (!String(rp[i].$.CurrencyCode).match(/^.{3}$/)) {
                throw 'invalid RatePlan: CurrencyCode must be a three letter string';
            }
        }
        // we don't allow mixing currencies
        if (currency === undefined) {
            currency = rp[i].$.CurrencyCode;
        } else {
            if (rp[i].$.CurrencyCode !== currency) {
                throw 'invalid RatePlan: CurrencyCode must be the same for all RatePlan elements';
            }
        }
...
function match_rates(arr, dep, eff_adult, eff_child, io, rpel, code, discount, num_free_kids, protocol_version, currency) {
    ...
    for (i = 0; i < a.length; i++) {
        rates.push(utilrate.get_rate(a[i], static_data, protocol_version));
    }
    ...
    while (utildate.date_diff(dt, dep) > 0) {
        ...
        if (rate_found) {
            ...
            if (discount.type_free_nights && utildate.date_diff(arr, dep) >= discount.nreq && rate_found_chunk === 1) {
                ...
                if (discount.patt !== undefined) {
                    ...
                    if (fn_index >= discount.nreq - discount.ndis) {
                        cost_details.push(round3(0.0) + ' ' + currency + ' for ' + dt + ' (' + chunk + ' nights) matched by rate ' +
            ...
            if (!fn_applied) {
                ...
                cost_details.push(round3(cost) + ' ' + currency + ' ' + s + 'for ' + dt + ' (' + chunk + ' nights) matched by rate ' +
    ...
    for (i = 0; i < codes.length; i++) {
        ...
        while (utildate.date_diff(dt, dep) > 0) {
            ...
            if (merged_supp.pre_dow === undefined || (merged_supp.pre_dow.split(""))[dowix] === "1" ) {
                ...
                cost_details.push(round3(0.0) + ' ' + currency + ' for "' + codes[i] + '" (not applicable due to ALPINEBITSDOW pattern) on ' + dt);
            }
            ...
            if (matching_amount != undefined) {
                ...
                switch (merged_supp.ctc) {
                    ...
                    case '19':  // per room per night
                        if (fn_hash[dt]) {
                            cost_details.push(round3(0.0) + ' ' + currency + ' for "' + codes[i] + '" (free nights discount applies) on ' + dt);
                        } else {
                            am = matching_amount;
                            cost_details.push(round3(am) + ' ' + currency + ' for "' + codes[i] + '" on ' + dt);
                            cost_total += am;
                        }
                        break;
                    case '12':  // per stay
                    case '18':  // per room per stay
                        ...
                        cost_details.push(round3(am) + ' ' + currency + ' for "' + codes[i] +
                            '" which is 1/' + stay_nights + ' of the amount per stay in this period (' + matching_amount + ' ' + currency + ') on ' + dt);
                        cost_total += am;
                        break;
                    case '20':  // per person per stay
                        am = matching_amount * stay_pax / stay_nights;
                        cost_details.push(round3(am) + ' ' + currency + ' for "' + codes[i] +
                            '" which is 1/' + stay_nights + ' of the amount per stay in this period (' + matching_amount + ' ' + currency + ') on ' + dt + ' times the guest count (' + stay_pax + ')');
                        cost_total += am;
                        break;
                    case '21':  // per person per night
                        if (fn_hash[dt]) {
                            ...
                            cost_details.push(round3(am) + ' ' + currency + ' for "' + codes[i] + '" on ' + dt + ' (' + matching_amount + ' ' + currency + ') times the guest count (' + stay_pax + ')');
                            cost_total += am;
                        }
                        break;
                    case '24':  // item (assumed to be 1)
                        am = matching_amount;
                        cost_details.push(round3(am) + ' ' + currency + ' for "' + codes[i] + '" (assuming the item count is 1)');
                        cost_total += am;
