...
function run(job) {
    ...
    if (job.protocol_version !== '2017-10' && job.protocol_version !== '2018-10') {
        throw 'job parameter: if given, protocol_version must be 2017-10 or 2018-10';
    }
    ...
    for (i = 0; i < plans.length; i++) {
        ...
        for (c = 0; c < rate_codes.length; c++) {
            ...
            var rmatch = match_rates(job.arrival, job.departure, eff_adult, eff_child, io, plans[i], rate_codes[c], discount, num_applied, job.protocol_version);
...
function match_rates(arr, dep, eff_adult, eff_child, io, rpel, code, discount, num_free_kids, protocol_version) {
