...
function get_discount(rpel) {
    ...
    // restriction -> min_advanced / max_advanced

    ret.restrictions.min_advanced = undefined; // default undefined (= unlimited)
    ret.restrictions.max_advanced = undefined;

    if (rule.$ && rule.$.MinAdvancedBookingOffset !== undefined) {
        var match;
        match = rule.$.MinAdvancedBookingOffset.match(/^P([0-9]+)D$/);
        if (match && !is_non_negative_int(match[1]) ) {
            throw 'invalid first Offer element: invalid MinAdvancedBookingOffset attribute';
        }
        ret.restrictions.min_advanced = Number(match[1]);
    }
    if (rule.$ && rule.$.MaxAdvancedBookingOffset !== undefined) {
        var match;
        match = rule.$.MaxAdvancedBookingOffset.match(/^P([0-9]+)D$/);
        if (match && !is_non_negative_int(match[1]) ) {
            throw 'invalid first Offer element: invalid MaxAdvancedBookingOffset attribute';
        }
        ret.restrictions.max_advanced = Number(match[1]);
    }
    ...
    for (i = 0; i < occupancy.length; i++) {
        ...
        if (ageq === '10') {
            ...
            if (minocc !== undefined) {
                if (!(is_non_negative_int(minocc) && minocc <= 99) ) {
