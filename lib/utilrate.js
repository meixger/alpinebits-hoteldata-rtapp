 ...
 */
function check_rates_overlap(rel_list, static_data, protocol_version) {

    var i, j;

    var a = [];
    for (i = 0; i < rel_list.length; i++) {
        a.push(get_rate(rel_list[i], static_data, protocol_version));
    }
 ...
 * @throws  {String}    in case of validation errors
 */
function get_rate(rel, static_data, protocol_version) {
    ...
    // the following 2017-10 restriction has been relaxed with 2018-10:
    if (protocol_version === '2017-10' && ret.add_amt.length > 0 && !ten_seen) {
